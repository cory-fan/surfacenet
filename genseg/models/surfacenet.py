...
class LocalGrouper(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        idx = pointnet2_utils.ball_query(self.radius, self.kneighbors, xyz, new_xyz)
...
class Model(nn.Module):
    def __init__(self, num_classes=50,points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 ...
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4], radius = [0.015,0.06,0.24,0.96],
...
def SurfaceNet(num_classes=3, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=32000, embed_dim=64, groups=1, res_expansion=0.5,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[1, 1, 1, 1],
                 k_neighbors=[48, 48, 48, 48], reducers=[8, 4, 4, 4], radius = [0.01,0.08,0.32,1.28],
                 de_dims=[512, 256, 128, 128], de_blocks=[2, 2, 2, 2],
                 gmp_dim=64, **kwargs)
