...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=3, help='batch size in training [default: 24]')
    ...
    parser.add_argument('--dataset', default='kumamoto', help='dataset name')
...
def test(model, loader, num_class=class_count, vote_num=1):
    ...
    for points, target in tqdm(loader):
        ...
        mean_correct.append(correct.item() / (batch_num_samples*args.num_point-(target.cpu()==3).sum()))
...
def main(args):
    ...
    if args.log_dir is None:
        experiment_dir = experiment_dir.joinpath(timestr+"_"+args.model+"_"+args.dataset)
    else:
    ...
    if (args.dataset=="kumamoto"):
        train_path = "data/kumamoto/train/"
        valid_path = "data/kumamoto/valid/"
    elif (args.dataset=="haiti"):
        train_path = "data/haiti/train/"
        valid_path = "data/haiti/valid/"
    else:
        print(args.dataset,"does not exist.")
        raise NotImplementedError
    TRAIN_DATASET = SegData(train_path, npoint=args.num_point)
    TEST_DATASET = SegData(valid_path, npoint=args.num_point,test_mode=True)
    trainDataLoader = DataLoader(TRAIN_DATASET, batch_size=args.batch_size, shuffle=True, num_workers=14,drop_last=True)
    testDataLoader = DataLoader(TEST_DATASET, batch_size=args.batch_size, shuffle=False, num_workers=14)
    ...
    classifier = nn.DataParallel(models.__dict__[args.model]().cuda())
    
    criterion = UTIL.get_loss(args.dataset).to(device)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for points, target in tqdm(trainDataLoader):
            ...
            correct_ = np.array(correct.item() / (batch_num_samples*args.num_point-(target.cpu()==3).sum()))
