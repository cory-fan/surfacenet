...
from data import ModelNet40, KumamotoEQ
...
def parse_args():
    ...
    parser.add_argument('--model', default='SurfaceNet', help='model name [default: pointnet_cls]')
    parser.add_argument('--epoch', default=600, type=int, help='number of epoch in training')
    parser.add_argument('--num_points', type=int, default=1024, help='Point Number')
    parser.add_argument('-lr', '--learning_rate', default=0.1, type=float, help='learning rate in training')
...
def fix_dict(state_dict):
    ...
    for key in state_dict:
        newdict[key.replace('.point_encoder','')]=state_dict[key]
    return newdict
...
def main():
    ...
    if args.msg is None:
        ...
        message = "-" + args.msg
    args.checkpoint = 'logs/' + args.model + message + '-' + str(args.seed)
    ...
    if ckpt_path!=None and os.path.isfile(ckpt_path):
        printf(f"Resuming last checkpoint from {ckpt_path}")
        ...
        try:
            ...
            net.load_state_dict(fix_dict(checkpoint['state_dict']),strict=False)
        start_epoch = checkpoint['epoch']
        logger = Logger(os.path.join(args.checkpoint, 'log.txt'), title="ModelNet" + args.model, resume=True)
    
    printf('==> Preparing data..')
    train_loader = DataLoader(KumamotoEQ(partition='train', num_points=args.num_points), num_workers=args.workers,
                              batch_size=args.batch_size, shuffle=True, drop_last=True)
    test_loader = DataLoader(KumamotoEQ(partition='valid', num_points=args.num_points), num_workers=args.workers,
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        printf(
            ...
            f"acc:{test_out['acc']}% time:{test_out['time']}s [best test avg acc: {best_test_acc_avg}%] \n\n")
    logger.close()
...
def train(net, trainloader, optimizer, criterion, device):
    ...
    for batch_idx, (data, label) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        logits = net(data)
        loss = criterion(logits, label, weights=[0.16,1])
        loss.backward()
...
def validate(net, testloader, criterion, device):
    ...
    with torch.no_grad():
        for batch_idx, (data, label) in enumerate(testloader):
            data, label = data.to(device), label.to(device).view(-1)
