...
def cal_loss(pred, target, smoothing=False,weights=None):
    ...
    if smoothing:
        ...
        if (weights!=None):
            batch_weights = torch.tensor([weights[int(i)] for i in target]).to(pred.device)
        else:
            batch_weights = torch.ones(pred.shape[0]).to(pred.device)

        one_hot = torch.zeros_like(pred).scatter(1, target.view(-1, 1), 1)
        one_hot = one_hot * (1 - eps) + (1 - one_hot) * eps / (n_class - 1)
        log_prb = F.log_softmax(pred, dim=1)
        
        loss = -(one_hot * log_prb * batch_weights.unsqueeze(-1)).sum(dim=1).mean()
    else:
        loss = F.cross_entropy(pred, target, reduction='mean', weight=torch.tensor(weights).cuda())
